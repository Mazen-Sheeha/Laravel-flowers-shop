...
@section('content')
    <div class="card p-2">
        <form class="card-body grid gap-5" method="POST" action="{{ route('adminProducts.update', $product->id) }}"
            id="store-product" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                ...
                <select name="category_id" class="select">
                    ...
                    @foreach ($categories as $category)
                        <option class="option" @selected(old('category_id') ? old('category_id') == $category->id : $product->category_id == $category->id) value="{{ $category->id }}">
            ...
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                ...
                <textarea name="desc" class="textarea" id="" cols="30" rows="8" name="desc"
                    placeholder="Product description">{{ old('desc') ? old('desc') : $product->desc }}</textarea>
            </div>
            <div class="card">
                                ...
                                <div class="text-gray-700 text-md pb-4">
                                    <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                                        ...
                                        <input class="input priceInput offer w-14 border-none" placeholder="20"
                                            type="text" name="offer"
                                            value="{{ old('offer') ? old('offer') : $product->offer }}" id="offer" />%
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <button type="submit" form="store-product" class="btn btn-primary flex justify-center m-5 mt-0">Update</button>
        </form>
    </div>
@endsection
@section('script')
        ...
        priceInputs.forEach(priceInput => {
            priceInput.addEventListener("input", (e) => {
                return e.target.value = isNaN(+e.target.value) ||
                    (e.target.name === "offer" ? e.target.value.length > 3 : e.target.value.length > 7) ||
                    (e.target.name === 'offer' ? e.target.value > 100 : +e.target.value > 9999) ?
